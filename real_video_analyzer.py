...
class PipettingVideoAnalyzer:
    def __init__(self, video_path, output_dir="real_analysis_results", glove_mode=True, hue_offset=90):
        ...
        self.protocol_config = {
            ...
            'repetitions_per_volume': 3,  # 3 times each volume for dyed water
            'expected_cycles': 16,  # 4 volumes × 4 liquids (3×DYE + 1×DYE-FREE + GLYCEROL + ETHANOL)
            'tip_changes': 4,  # After each liquid type
            'reverse_pipetting_liquids': ['GLYCEROL'],  # Liquids requiring reverse pipetting
            'pre_wet_liquids': ['ETHANOL']  # Liquids requiring pre-wetting
        }
